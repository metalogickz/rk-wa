...
// Настройка окружения для тестов SQLite
beforeAll(async () => {
  // Переключаем провайдер базы данных на SQLite
...
// Тесты WhatsApp API с SQLite
describe('WhatsApp API Tests (SQLite)', () => {
  ...
  // Тесты для обработки JSON данных в SQLite
  describe('JSON data handling in SQLite', () => {
    it('should correctly process metadata JSON in messages', async () => {
      // Запрос к API для проверки обработки JSON в SQLite
      // Это может быть тест, который сохраняет сообщение с метаданными
      // и затем проверяет правильность их обработки
      
      // Например:
      // const res = await request(app)
      //   .post(`/api/instances/${testInstanceId}/test-message`)
      //   .set('x-api-key', testApiKey)
      //   .send({
      //     metadata: { test: 'value', nested: { data: true } }
      //   });
      // 
      // expect(res.statusCode).toEqual(200);
      // expect(res.body.metadata).toHaveProperty('test');
      // expect(res.body.metadata.nested).toHaveProperty('data');
